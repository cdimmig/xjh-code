...
class MLPPolicy(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
	...
	# baseline is the expected reward
        if nn_baseline:
    ...
    def get_action(self, obs: np.ndarray) -> np.ndarray:
        ...
        # self.forward(ptu.from_numpy(observation)) is a distribution
        # .sample() is a sample follows that distribution
    ...
    def forward(self, observation: torch.FloatTensor):
        # TODO: get this from hw1
        # HW1 returns action
        # HW2 returns a distribution of actions
        if self.discrete:
            network_outputs = self.logits_na(observation)
            network_outputs = nn.functional.log_softmax(network_outputs).exp()
            return distributions.Categorical(network_outputs)
        ...
        #return distributions.MultivariateNormal(self.mean_net(observation), scale_tril = torch.diag(self.logstd))
...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(self, observations, actions, advantages, q_values=None):
        ...
        #print(log_pi.shape)
        # in this case loss is a number
        
        #loss = torch.neg(torch.mean(torch.mul(log_pi, advantages)))
        loss = torch.neg(torch.sum(torch.mul(log_pi, advantages)))
